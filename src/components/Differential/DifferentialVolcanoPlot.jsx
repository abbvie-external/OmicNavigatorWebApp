...
class DifferentialVolcanoPlot extends React.PureComponent {
    ...
    if (hovered != null) {
      ...
      if (hoveredCircle != null) {
        ...
        if (hoveredCircle.attributes['class'].value.endsWith('selected')) {
          ...
          console.log(hoveredCircle.attributes['r']);
        }
        // }
        this.setState({
          hoveredCircleData: {
  ...
  getToolTip() {
    ...
    if (hovering) {
      if (hoveredElement === 'bin') {
        ...
        console.log('hoveredElement', bin);
        const xstat = bin
        ...
        d3.select('svg')
          ...
          .attr('y', hoveredCircleData.position[1] * 1 + 10)
          .attr('id', 'tooltip')
          .append('rect')
          .attr('width', '100%')
          .attr('height', '100%')
  ...
  updateVoclanoAfterUpsetFilter(data, xScale, yScale) {
    ...
    if (isUpsetVisible) {
      ...
      if (data.length >= 5000 && elementsToDisplay.length >= 5000) {
      ...
      } else if (data.length >= 5000 && elementsToDisplay.length < 5000) {
        self.renderCirclesFilter(unfilteredObject.circles);
        self.renderBinsFilter(unfilteredObject.bins);
