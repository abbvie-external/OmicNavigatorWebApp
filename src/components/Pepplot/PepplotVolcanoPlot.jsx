...
class PepplotVolcanoPlot extends Component {
  ...
  shouldComponentUpdate(nextProps, nextState) {
    const volcanoActive =
      this.props.activeIndexPepplotView === 1 ? true : false;
    return volcanoActive;
  }
  ...
  componentDidMount() {
    ...
    if (pepplotResults.length !== 0) {
      ...
      if (pepKeys.includes('id')) {
        id = 'id';
      } else if (pepKeys.includes('id_mult')) {
        id = 'id_mult';
      }
      if (pepKeys.includes('logFC')) {
        xAxis = 'logFC';
      } else if (pepKeys.includes('F')) {
        xAxis = 'F';
      } else if (pepKeys.includes('t')) {
        xAxis = 't';
      }
      if (pepKeys.includes('adj_P_Val')) {
        yAxis = 'adj_P_Val';
      } else if (pepKeys.includes('P_Value')) {
        yAxis = 'P_Value';
      }
      this.setState({
        identifier: id,
      ...
      if (d3.event.selection !== undefined && d3.event.selection !== null) {
        ...
        const isBrushed = function(x, y) {
          const brushTest =
            ...
            brushedCircles[0][1] <= y && y <= brushedCircles[1][1];
          return brushTest;
        };
