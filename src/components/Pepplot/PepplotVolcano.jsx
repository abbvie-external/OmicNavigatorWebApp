...
import { Form, Grid, Select, Checkbox, Dimmer, Loader } from 'semantic-ui-react';
...
class PepplotVolcano extends Component{
    ...
    componentDidUpdate(prevProps, prevState) {
      ...
      if(this.props.selectedFromTableData.length === 1 && this.state.volcanoWidth !== 400){
        this.setState({volcanoWidth:400})
      }else if(this.props.selectedFromTableData.length !== 1 && this.state.volcanoWidth !== 600){
        this.setState({volcanoWidth:600})
      }
    }
    getAxisLabels=()=>{
      if(this.props.pepplotResults.length !== 0){
        const configCols = this.props.pepplotColumns;
        const configColsArr = configCols.map(e=>{return e.title});
        configColsArr.shift()
        const index = configColsArr.indexOf('Set_Membership');
        if (index > -1) {configColsArr.splice(index, 1);}
        if(configColsArr.includes("logFC")){
          this.handleDropdownChange({},{name:"xAxisSelector", value:"logFC"})
        }else{
          this.handleDropdownChange({},{name:"xAxisSelector", value:configColsArr[0]})
        }
        if(configColsArr.includes("adj_P_Val")){
          this.setState({
            yAxisLabel: "adj_P_Val",
            doYAxisTransformation: true
          })
        } else if(configColsArr.includes("P_Value")){
          this.setState({
            yAxisLabel: "P_Value",
            doYAxisTransformation: true
          })
        }else {
          this.handleDropdownChange({},{name:"yAxisSelector", value:configColsArr[configColsArr.length-1]})
        }
        if(Object.keys(this.props.pepplotResults[0]).includes("id_mult")){
          this.setState({identifier: "id_mult"})
        }else{
          this.setState({identifier: "id"})
        }
        const axes = configColsArr.map(e =>{
          return({
            key: e,
            text: e,
            value: e,
    ...
    handleDropdownChange(evt, { name, value }){
      ...
      if(name === "xAxisSelector"){
        this.setState({
          xAxisLabel: value,
          doXAxisTransformation: false,
          allowXTransformation:this.getMaxAndMin(pepplotResultsUnfiltered, value)[0]>0
        })
      } else {
        this.setState({
          yAxisLabel: value,
          doYAxisTransformation: false,
          allowYTransformation:this.getMaxAndMin(pepplotResultsUnfiltered, value)[0]>0
        })
      }
    }
    handleTransformationChange=(evt, { name })=>{
      if(name==="xTransformationCheckbox"){
        this.setState({doXAxisTransformation: !this.state.doXAxisTransformation})
      } else {
        this.setState({doYAxisTransformation: !this.state.doYAxisTransformation})
      }
    }
    getSVGPlot=()=>{
      if(this.props.selectedFromTableData.length !== 1){
        return null
      } else if(this.props.selectedFromTableData.length === 1 && !this.props.isProteinSVGLoaded){
        return(
    ...
    render(){
        ...
        if(!isItemSelected){
        const xAxisTransformBox = allowXTransformation?<span title="-log10 Transform"><Form.Field
          control={Checkbox}
          name="xTransformationCheckbox"
          checked={doXAxisTransformation}
          onClick={this.handleTransformationChange.bind(this)}
        ></Form.Field></span>:null
        const yAxisTransformBox = allowYTransformation?<span title="-log10 Transform"><Form.Field
          control={Checkbox}
