if (!window.jQuery) {
  alert(
    'Could not find jQuery! The HTML must include jquery.js before opencpu.js!'
  );
}
(function($) {
  var r_cors = false;
    ...
    var jqxhr = $.ajax(settings)
      .done(function() {
        var loc =
          ...
          console.log('Location response header missing.');
        var key =
          jqxhr.getResponseHeader('X-ocpu-session') ||
          console.log('X-ocpu-session response header missing.');
      ...
      .fail(function() {
        console.log(
          'OpenCPU error HTTP ' + jqxhr.status + '\n' + jqxhr.responseText
        );
      });
    return jqxhr;
  }
  function r_fun_call_json(fun, args, handler) {
    return r_fun_ajax(
      fun,
      { data: JSON.stringify(args || {}), contentType: 'application/json' },
      handler
    );
  }
  ...
  function initplot(targetdiv) {
    ...
    var ocpuplot = (function() {
      ...
      var plotdiv = $('<div />')
        .attr({
          style:
            'width: 100%; height:100%; min-width: 100px; min-height: 100px; position:relative; background-repeat:no-repeat; background-size: 100% 100%;'
        })
        .appendTo(targetdiv)
      ...
      var spinner = $('<span />')
        .attr({
          style:
            'position: absolute; top: 20px; left: 20px; z-index:1000; font-family: monospace;'
        })
        .text('loading...')
        .appendTo(plotdiv);
      var pdf = $('<a />')
        .attr({
          target: '_blank',
          style:
            'position: absolute; top: 10px; right: 10px; z-index:1000; text-decoration:underline; font-family: monospace;'
        })
        .text('pdf')
        .appendTo(plotdiv);
      var svg = $('<a />')
        .attr({
          target: '_blank',
          style:
            'position: absolute; top: 30px; right: 10px; z-index:1000; text-decoration:underline; font-family: monospace;'
        })
        .text('svg')
        .appendTo(plotdiv);
      var png = $('<a />')
        .attr({
          target: '_blank',
          style:
            'position: absolute; top: 50px; right: 10px; z-index:1000; text-decoration:underline; font-family: monospace;'
        })
        .text('png')
        .appendTo(plotdiv);
      function updatepng() {
      ...
      function setlocation(newloc) {
        ...
        if (!Location) {
          ...
          pdf
            .attr(
              'href',
              Location + 'graphics/last/pdf?width=11.69&height=8.27&paper=a4r'
            )
            .show();
          svg
            .attr('href', Location + 'graphics/last/svg?width=11&height=6')
            .show();
          png
            .attr('href', Location + 'graphics/last/png?width=800&height=600')
            .show();
          updatepng();
        }
      }
  ...
  function testhtml5() {
    if (window.FormData === undefined) {
      alert(
        'Uploading of files requires HTML5. It looks like you are using an outdated browser that does not support this. Please install Firefox, Chrome or Internet Explorer 10+'
      );
  ...
  function seturl(newpath) {
    if (!newpath.match('/R$')) {
      alert(
        'ERROR! Trying to set R url to: ' +
          newpath +
          ". Path to an OpenCPU R package must end with '/R'"
      );
    } else {
      ...
      if (
        window.location.protocol != r_path.protocol ||
        window.location.host != r_path.host
      ) {
        r_cors = true;
        if (!('withCredentials' in new XMLHttpRequest())) {
          alert(
            'This browser does not support CORS. Try using Firefox or Chrome.'
          );
        }
      }
      if (window.location.protocol == 'https:' && r_path.protocol != 'https:') {
        alert(
          'Page is hosted on HTTPS but using a (non-SSL) HTTP OpenCPU server. This is insecure and most browsers will not allow this.'
        );
      }
      if (r_cors) {
